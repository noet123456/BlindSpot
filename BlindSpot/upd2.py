...
# ==========================================
# Quay màn hình (Bỏ comment 3 dòng code dưới để quay video)
# fourcc = cv2.VideoWriter_fourcc(*'mp4v') 
# final_video = cv2.VideoWriter('finale.mp4', fourcc, 20.0, (1280, 720))
# raw_video = cv2.VideoWriter('raw.mp4', fourcc, 20.0, (1280, 720))
# ==========================================
while True:
    success, frame = cap.read()
    if not success:
        break
    # Bỏ comment để quay video
    ...
    # Sử dụng vòng lặp chạy từng ảnh để tránh lỗi ngộp bộ nhớ (Batch Size)
    results = []
    ...
    # 4.2. Vẽ đồ họa lên từng khung hình nhỏ và kích hoạt âm thanh
    for i, result in enumerate(results):
        ...
        if i == target_cam_idx and target_rect_corners is not None and current_distance > 0:
            ...
            print(f"[CẢNH BÁO] {class_name.upper()} tại CAM {i + 1} | Area: {global_max_area:.0f} px^2 | Dist: {current_distance} cm")
    ...
    # Bỏ comment để quay video
    # finale_video.write(display_frame)
